  ...
  {   		
	//ros::spinOnce();//以下兩行為有間隔的(10hz)等待topic發布消息(撞牆)***
		...
		*/
		...
		int input=0;
		printf("\nInput your command(0~5):小心!若輸入非int之值，會出現bug!\n");
		scanf("%d", &input);	//小心!若輸入非int之值，會出現bug!
		...
		ROS_INFO("A:%0.2f|%0.2f|%0.2f|%0.2f|%0.2f|%0.2f",
			...
			//msg.trapezium=fabs(20-(Rthumb/(range[0][0]-range[0][1]))*10)//range to open & half status
			
			msg.thumb=poses[input][0],	//control by commands
			msg.index=poses[input][1],			
			msg.middle=poses[input][2],			
			msg.ring=poses[input][3],	
			msg.baby=poses[input][4],
			msg.trapezium=poses[input][5]
